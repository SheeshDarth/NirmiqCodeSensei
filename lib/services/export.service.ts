...
export async function generateWorkspaceMarkdown(
  ...
  lines.push("");
  lines.push(
    `> Exported from CodeSensei on ${formatDate(Date.now(), LONG_DATE)}`
  );
  lines.push("");
  ...
  lines.push(hr());
  lines.push(
    `_Generated by [CodeSensei](https://github.com/SheeshDarth/NirmiqLearnOS) — Build with AI, but learn like a real engineer._`
  );

  const markdown = lines.join("\n");
  const filename = `codesensei-${slug(ws.title)}-${Date.now()}.md`;
