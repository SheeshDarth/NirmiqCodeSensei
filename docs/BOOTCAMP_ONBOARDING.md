# NirmiqCodeSensei — Bootcamp Onboarding Guide

This guide is for bootcamp instructors and program managers setting up NirmiqCodeSensei
...
| NCS_PRO_KEY | Provided by instructor (from Gumroad bulk purchase) |
...
1. **Student writes a function** → AI calls `ncs_generate_questions` → 5 explain-back questions saved to workspace
2. **Student implements a feature** → AI calls `ncs_suggest_concepts` → DSA/CS concepts linked with practice tasks
3. **Student hits an error** → AI calls `ncs_debug_assist` → structured diagnosis saved; `add_debug_log` creates a permanent record
...
4. Each student sets `NCS_PRO_KEY=their-key` in `.env.local`
...
- All data is stored locally on each student's machine in `data/nirmiqcodesensei.db`
- Nothing is sent to NirmiqCodeSensei servers — zero telemetry
